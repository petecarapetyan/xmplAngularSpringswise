...
type RestOf<T extends ITicket | NewTicket> = Omit<T, 'timeStamp'> & {
...
export class TicketService {
  ...
  protected convertDateFromClient<T extends ITicket | NewTicket | PartialUpdateTicket>(ticket: T): RestOf<T> {
    return {
      ...ticket,
      timeStamp: ticket.timeStamp?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restTicket: RestTicket): ITicket {
    return {
      ...restTicket,
      timeStamp: restTicket.timeStamp ? dayjs(restTicket.timeStamp) : undefined,
    };
  }
